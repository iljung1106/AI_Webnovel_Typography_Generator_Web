// @ts-nocheck
import { createMaterialTextureCanvas } from "./material-textures";
...
float maskAt(vec2 uv) {
  ...
  float alphaMask = s.a;
  float darkness = 1.0 - dot(s.rgb, vec3(0.299, 0.587, 0.114));
  float blackMask = pow(smoothstep(0.20, 0.68, darkness), 1.45);
  blackMask *= smoothstep(0.02, 0.18, s.a);
  float inkAlphaMask = alphaMask * blackMask;

  if (u_maskMode == 1) {
    return blackMask;
  }
  if (u_maskMode == 2) {
    return max(inkAlphaMask, blackMask);
  }
  return alphaMask;
}
